...
def setup_buttons(self):
    ...
    Handles the creation of buttons and other widgets that work like ones.

    Not all widgets which work as buttons are created in this method.
    Entries, which also work as buttons, are later created in another method
    and stored in the same list as the buttons created here.
    """
    ### create a special list to hold buttons; the list is stored in its own
    ### attribute, as well as referenced locally
    buttons = self.buttons = List2D()

    ### now proceed creating and appending all different buttons
    ...
    color_add_option_menu = self.color_add_option_menu = (
    ...
    ## a seq sorting button widget used to sort the properties used for
    ## sorting colors as desired
    ##
    ## the order of the available items are not relevant, since they are
    ## only taken into account when inserted in the list used as the 'value'
    ## argument
    ...
    color_sorting_holder = self.color_sorting_holder = (
    ...
    ## checkbutton to use as a flag so the user can specify whether to use
    ## alpha or not, accompanied by a label which will work as a button

    ## checkbutton

    alpha_checkbutton = self.alpha_checkbutton = (
    ...
    ### now that all buttons are created, assign a command to their
    ### 'on_mouse_release' attribute (except the checkbutton, which receives
    ### the command in its 'command' attribute)
    ###
    ### note that the commands assigned to 'on_mouse_release' are passed
    ### through get_oblivious_callable() before being assigned;
    ###
    ### this is done so the commands ignore the event object they receive when
    ### the buttons are clicked, because since they originally don't have any
    ### parameters, they would raise an exception when receiving the event as
    ### an argument
    ###
    ### (only commands which originally accept such event object wouldn't need
    ...
    move_left_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.widget_to_left)
    )

    ## "move right" button

    move_right_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.widget_to_right)
    )

    ## "remove color" button

    remove_color_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.remove_color_widgets)
    )

    ## "add color" button

    add_color_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.add_new_color_widget)
    )

    ## sort button

    sort_colors_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.sort_colors)
    )


    ## "reverse order" button

    reverse_order_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.reverse_color_order)
    )

    ## shuffle button

    shuffle_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.shuffle_color_order)
    )

    ## "remove duplicates" button

    remove_duplicates_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.remove_duplicated_colors)
    )

    ## "html colors" button

    html_colors_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.pick_html_colors)
    )

    ## "pygame colors" button

    pygame_colors_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.pick_pygame_colors)
    )

    ## "import" button

    import_colors_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.import_colors)
    )

    ## "export" button

    export_colors_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.export_colors)
    )
    ...
    view_button.on_mouse_release = (
        get_oblivious_callable(colors_panel.view_colors)
    )
