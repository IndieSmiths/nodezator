...
def setup_entries(self):
    ...
    Handles the creation of entry widgets.

    Since entry widgets also work as buttons, they are stored in the
    list of buttons in the 'buttons' attribute at the end.
    """
    ### instantiate entries
    ...
    ## iterate over pairs formed by each scale and its respective operation
    ## to build an entry for each scale

    self.scale_entries = scale_entries = [
    ...
    self.entry_map = {
        'hls': scale_entries[:3],
        'hsv': [scale_entries[i] for i in (0, 2, 3)],
        'rgb': scale_entries[4:7],
        'alpha': scale_entries[7],
    }
    ...
    ### add entries to the buttons custom list, since they work as buttons too
    ### (they have a mouse action method)

    self.buttons.extend(scale_entries)
